...
export default function AddProductScreen() {
  ...
  return (
    <Screen title="Add product" subtitle="Add an item to your inventory" back>
      <PhotoPicker uri={photoUri} onPick={setPhotoUri} label="Add product photo" />
      <Field label="Product name" value={name} onChangeText={setName} placeholder="e.g.Rice 5kg" editable={!isSaving} />
      <Field label="Cost price" value={cost} onChangeText={setCost} keyboardType="numeric" placeholder="0" editable={!isSaving} />
      <Field label="Selling price" value={price} onChangeText={setPrice} keyboardType="numeric" placeholder="0" editable={!isSaving} />
      <Field label="Quantity" value={quantity} onChangeText={setQuantity} keyboardType="numeric" placeholder="0" editable={!isSaving} />
      <Field label="Low stock threshold" value={threshold} onChangeText={setThreshold} keyboardType="numeric" placeholder="5" editable={!isSaving} />
      {error ? <Text style={styles.error}>{error}</Text> : null}
      <PrimaryButton label={isSaving ? 'Saving...' : 'Save product'} onPress={save} disabled={isSaving} />
    </Screen>
  );
}
